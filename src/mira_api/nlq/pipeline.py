...
def missing_country_warning(
    ...
    *,
    ...
    """Describe los paises presentes en las filas, sin diagnosticar su ausencia.

    Si se alcanzo un limite, puede haber coincidencias fuera del resultado.
    Las consultas con montos conservan su aviso separado de monedas.
    """
    ...
    if len(pedidos) >= 2 and faltantes:
        names = {
            ...
            "NI": "Nicaragua", "SV": "El Salvador", "PA": "Panamá",
        }
        present_names = ", ".join(names.get(c, c) for c in sorted(presentes))
        missing_names = ", ".join(names.get(c, c) for c in faltantes)
        return Warning(
            code="MISSING_COUNTRY_IN_RESULT",
            message_es=(
                f"Entre los {len(rows)} resultados mostrados aparecen {present_names}. "
                + (
                    f"{missing_names} podrían tener coincidencias fuera de este límite."
                    if limited else
                    f"No aparecen resultados de {missing_names} con estos filtros; "
                    "esto no significa que falten datos de esos países en general."
                )
            ),
            message_en=(
                f"The {len(rows)} results shown include {present_names}. "
                + (
                    f"{missing_names} may have matches beyond this limit."
                    if limited else
                    f"No results from {missing_names} appear with these filters; "
                    "this does not mean those countries lack data in general."
                )
            ),
            details={"paises_ausentes": faltantes, "paises_presentes": sorted(presentes)},
        )
    ...
    *,
    ...
    try:
        ...
        if rows_result.row_count > 0:
            ...
            falta_pais = missing_country_warning(
                columns, rows_result.rows, countries,
                limited=rows_result.truncated or limit_reached,
            )
